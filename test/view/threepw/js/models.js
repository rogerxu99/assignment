...
window.onload = function() {
    ...
    $.get("https://users.sussex.ac.uk/~zx84//3dapp/assignment/test/controller/ImageControllerApi.php?type=2", function (data, status) {
        ...
        data.forEach(e=>{
            $('.imgsZ').append(
                `<div class="img-item" style="background: url('https://users.sussex.ac.uk/~zx84/3dapp/assignment/test/controller/ImageView.php?id=${e.id}');background-size: 100% 100%" onclick="imgClick()"></div>`
            )
        })
    })
    changeName('coke')
